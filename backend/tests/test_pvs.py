...
def test_verifier_integrite_pv_valide():
    ...
    # Altération d'une donnée immuable
    pv_obj.montant = 0
...
# ── Tests Sprint Beta — hash permis ──────────────────────────────────────


def test_hash_permis_coherent():
    """Même permis = même hash — recherche citoyen cohérente."""
    permis = "SN-2024-12345"
    hash1 = hashlib.sha256(permis.encode()).hexdigest()
    hash2 = hashlib.sha256(permis.encode()).hexdigest()
    assert hash1 == hash2
    assert len(hash1) == 64


def test_hash_permis_different():
    """Deux permis différents = hashs différents."""
    hash1 = hashlib.sha256(b"SN-2024-12345").hexdigest()
    hash2 = hashlib.sha256(b"SN-2024-99999").hexdigest()
    assert hash1 != hash2


def test_recherche_par_hash_permis():
    """preparer_pv stocke le hash du permis — jamais le permis en clair."""
    num_permis_hash = hashlib.sha256(b"SN-2024-12345").hexdigest()
    pv = preparer_pv(
        agent_id=1,
        num_permis_chiffre="Y2hpZmZyZW1lbnRfbm9tYnJlX3Blcm1pc18xMjM0NQ==",
        iv="MTIzNDU2Nzg5MDEy",
    ...
    assert pv["num_permis_hash"] == num_permis_hash
    assert "num_permis_chiffre" in pv
    assert "SN-2024-12345" not in str(pv)