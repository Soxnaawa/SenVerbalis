...
## 5. État d'Avancement de la Livraison (Sprint Beta & Version Finale)

Conformément aux directives du cours, le sprint **Beta** et la livraison finale se concentrent sur la consolidation des mécanismes d'**Authentification** et d'**Autorisation**, ainsi que sur la **Containerisation**. Nous avons finalisé l'ensemble des exigences fonctionnelles et sécuritaires :

*   **Audit & Traçabilité (100% Terminé)** : Journal d'audit immuable en base de données avec déclencheurs SQL (triggers), logs unifiés avec adresses IP, et module d'audit dans la console d'administration.
*   **Authentification & Autorisation (100% Terminé)** : Authentification JWT stockée en mémoire (anti-XSS), politique RBAC stricte côté serveur (Agent, Superviseur, Administrateur) et déconnexion automatique sur inactivité.
*   **Confidentialité & Intégrité (100% Terminé)** : Chiffrement local client AES-GCM (Web Crypto API) et signature HMAC-SHA256 côté serveur avec validation en temps réel de l'inaltérabilité physique.
*   **Déclencheurs SQL de Sécurité (100% Terminé)** : Triggers robustes appliqués au niveau des tables `pvs` (seul le statut est mutable) et `audit_logs` (interdiction stricte de modifier ou supprimer) pour SQLite (local) et PostgreSQL (production).
*   **Containerisation & Orchestration (100% Terminé)** : Configuration multi-conteneurs via Docker Compose (PostgreSQL, FastAPI Backend, React Frontend servi via Nginx) dans un réseau bridge sécurisé avec isolation de la base de données.
